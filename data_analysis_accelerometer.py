...
def plot(t, signal, freq, PSD, L, ffilt):
    ...
    plt.sca(axs[0])
    plt.plot(t, signal, color='r', LineWidth=1.5, label='gFz')
    plt.xlim(t[0], t[-1])
...
def preprocess(t, gFz, dt, length):
    t = np.zeros(length)
    gFz = np.zeros(length)
    index = 0
